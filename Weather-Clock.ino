...
const long interval = 5000;        // 事件间隔，这里设置为 1 秒
...
const char* ssid = "sdu_net";  // 连接WiFi名（此处使用XXX为示例）
...
void TandW() {
  ...
  if (previousMillis >= 5000 && currentMillis - previousMillis <= 15000) {
    ...
    String reqRes = "/v3/weather/now.json?key=" + Info::reqUserKey + +"&location=" + reqLocation + "&language=en&unit=" + reqUnit;
...
void threeday() {
  // 建立心知天气API当前天气请求资源地址
  String reqRes = "/v3/weather/daily.json?key=" + Info::reqUserKey + +"&location=" + reqLocation + "&language=en&unit=" + reqUnit + "&start=0&days=3";
...
void reminder() {
  ...
  String reqRes = "/";
  String httpRequest = String("GET ") + reqRes + " HTTP/1.1\r\n" + "Host: " + Info::reminder_host + "\r\n" + "Connection: close\r\n\r\n";
  Serial.println("");
  Serial.print("Connecting to ");
  Serial.print(host);
  WiFiClient client;
  // 尝试连接服务器
  if (client.connect(Info::reminder_host, 80)) {
